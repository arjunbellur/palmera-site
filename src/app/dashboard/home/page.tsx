  ...
  return (
    <div>
      <div style={{ marginBottom: '2rem' }}>
        ...
        <p style={{ fontFamily: 'var(--font-serif)', color: 'var(--db-text-muted)', fontSize: '0.9375rem', margin: 0, letterSpacing: '0.02em' }}>
      ...
      <div style={{ background: 'var(--db-bg-card)', border: '1px solid var(--db-border)', borderRadius: '0.5rem', padding: '1.25rem 1.5rem', marginBottom: '2rem', display: 'flex', alignItems: 'center', gap: '1.25rem' }}>
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.625rem' }}>
            <span style={{ fontSize: '0.8125rem', color: 'var(--db-text-muted)', fontFamily: 'var(--font-sans)' }}>Onboarding progress</span>
            <span style={{ fontSize: '0.8125rem', color: '#be9a56', fontFamily: 'var(--font-sans)' }}>{completedCount} / {SECTIONS.length} complete</span>
          </div>
          <div style={{ height: '0.3125rem', background: 'var(--db-border-subtle)', borderRadius: '0.1875rem', overflow: 'hidden' }}>
        ...
        <div style={{ fontFamily: 'var(--font-display)', color: progressPct === 100 ? '#be9a56' : 'var(--db-text-muted)', fontSize: '1.625rem', flexShrink: 0 }}>{progressPct}%</div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 18rem), 1fr))', gap: '1rem' }}>
        {SECTIONS.map(section => {
          const basicsComplete = sections.basics === 'complete'
          const locked =
            (['listings', 'photos', 'operations'].includes(section.key) && !basicsComplete) ||
            (['documents', 'signoff'].includes(section.key) && !mustHaveDone)
          return (
            <SectionCard key={section.key} title={section.title} description={section.description}
              ...
              priority={section.priority} href={section.href} locked={locked} />
          )
        })}
      </div>
    </div>

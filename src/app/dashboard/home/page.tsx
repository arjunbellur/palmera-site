    ...
    <div>
      <div style={{ marginBottom: '2rem' }}>
        ...
        <p style={{ fontFamily: 'var(--font-serif)', color: 'var(--db-text-muted)', fontSize: '0.9375rem', margin: 0 }}>
          Manage the businesses you run on Palmera. Each company has its own listings and payouts.
        </p>
      </div>
      ...
      {!signed && (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem', flexWrap: 'wrap', background: 'var(--db-bg-banner)', border: '1px solid rgba(158,118,59,0.25)', borderRadius: '0.5rem', padding: '1rem 1.25rem', marginBottom: '1.75rem' }}>
          <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.8125rem', color: 'var(--db-text-muted)', margin: 0, lineHeight: 1.5 }}>
            Sign the Palmera Partner Agreement to activate your account. One signature covers all your companies.
          </p>
          <a href="/dashboard/settings" style={{ ...addBtn, flexShrink: 0 }}>Review &amp; sign →</a>
        </div>
      )}

      {/* Companies */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem', marginBottom: '1rem' }}>
        <h2 style={{ fontFamily: 'var(--font-serif)', color: 'var(--db-text)', fontSize: '1.0625rem', fontWeight: 400, margin: 0 }}>Your companies</h2>
        {companies.length > 0 && <a href="/dashboard/companies/new" style={addBtn}>+ Add company</a>}
      </div>
      ...
      {companies.length === 0 ? (
        <div style={{ background: 'var(--db-bg-card)', border: '1px dashed var(--db-border-dashed)', borderRadius: '0.625rem', padding: '3rem 2rem', textAlign: 'center' }}>
          <p style={{ fontFamily: 'var(--font-serif)', color: 'var(--db-text-faint)', fontSize: '1.0625rem', margin: '0 0 0.5rem' }}>No companies yet</p>
          <p style={{ fontFamily: 'var(--font-sans)', color: 'var(--db-text-ghost)', fontSize: '0.8125rem', margin: '0 0 1.5rem' }}>Add your first business to start listing experiences on Palmera.</p>
          <a href="/dashboard/companies/new" style={addBtn}>+ Add your first company</a>
        </div>
      ) : (
            ...
            return (
              <a key={c.id} href={`/dashboard/companies/${c.id}`}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.5rem' }}>
                  <h3 style={{ fontFamily: 'var(--font-serif)', color: 'var(--db-text)', fontSize: '1rem', fontWeight: 500, margin: 0, letterSpacing: '0.02em' }}>{c.name || 'Untitled company'}</h3>
                  {c.active && <span style={{ fontSize: '0.5625rem', color: '#9e763b', border: '1px solid rgba(158,118,59,0.4)', padding: '0.125rem 0.4375rem', borderRadius: '2rem', fontFamily: 'var(--font-sans)', letterSpacing: '0.06em', textTransform: 'uppercase' }}>Active</span>}
                </div>
                <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.75rem', color: 'var(--db-text-faint)', margin: '0 0 1rem', textTransform: 'capitalize' }}>
                  {[c.category, c.city].filter(Boolean).join(' · ') || 'Set up your profile'}
                </p>

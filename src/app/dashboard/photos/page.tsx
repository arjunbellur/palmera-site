...
export default function PhotosPage() {
  ...
  const badge = (label: string, gold = false) => (
    <span style={{ fontSize: '0.625rem', color: gold ? '#be9a56' : 'rgba(223,201,166,0.65)', border: `1px solid ${gold ? 'rgba(190,154,86,0.4)' : 'rgba(223,201,166,0.25)'}`, padding: '0.125rem 0.5rem', borderRadius: '0.1875rem', fontFamily: 'var(--font-sans)', letterSpacing: '0.06em', textTransform: 'uppercase' as const }}>{label}</span>
  )
  ...
  return (
    <div>
      <div style={{ marginBottom: '2rem' }}>
        ...
        <h1 style={{ fontFamily: 'var(--font-display)', color: '#dfc9a6', fontSize: 'clamp(1.375rem, 3vw, 1.625rem)', fontWeight: 400, letterSpacing: '0.06em', margin: '0 0 0.375rem' }}>Visuals</h1>
        <p style={{ fontFamily: 'var(--font-serif)', color: 'rgba(223,201,166,0.72)', fontSize: '0.875rem', margin: 0 }}>Great photos are the single biggest factor in bookings. No filters, natural light, original quality.</p>
      </div>
      <div style={{ background: 'rgba(158,118,59,0.06)', border: '1px solid rgba(158,118,59,0.15)', borderRadius: '0.5rem', padding: '1rem 1.25rem', marginBottom: '2rem' }}>
        <p style={{ fontSize: '0.6875rem', color: 'rgba(223,201,166,0.75)', fontFamily: 'var(--font-sans)', margin: '0 0 0.5rem', letterSpacing: '0.06em', textTransform: 'uppercase' }}>Photo standards</p>
        <ul style={{ margin: 0, padding: '0 0 0 1rem', listStyleType: 'disc' }}>
          {['Shot horizontally on a recent phone — no photographer needed', 'Natural light only, no flash', 'No filters, no Instagram crops', 'Send at ORIGINAL quality — never screenshots'].map(tip => (
            <li key={tip} style={{ fontSize: '0.8125rem', color: 'rgba(223,201,166,0.72)', fontFamily: 'var(--font-sans)', marginBottom: '0.1875rem', lineHeight: 1.5 }}>{tip}</li>
          ))}
        </ul>
      </div>
      <div style={{ marginBottom: '2rem' }}>
        <h2 style={{ fontFamily: 'var(--font-serif)', color: '#dfc9a6', fontSize: '1.0625rem', fontWeight: 400, margin: '0 0 1.25rem', display: 'flex', alignItems: 'center', gap: '0.625rem' }}>
      ...
      <div style={{ marginBottom: '2rem' }}>
        <h2 style={{ fontFamily: 'var(--font-serif)', color: '#dfc9a6', fontSize: '1.0625rem', fontWeight: 400, margin: '0 0 0.5rem', display: 'flex', alignItems: 'center', gap: '0.625rem' }}>
          Gallery photos {badge('First month')}
        </h2>
        <p style={{ fontSize: '0.8125rem', color: 'rgba(223,201,166,0.65)', fontFamily: 'var(--font-sans)', margin: '0 0 1.25rem' }}>Aim for variety — people enjoying, the space, food/equipment, golden-hour shots.</p>
      ...
      <div style={{ marginBottom: '2rem' }}>
        <h2 style={{ fontFamily: 'var(--font-serif)', color: '#dfc9a6', fontSize: '1.0625rem', fontWeight: 400, margin: '0 0 1.25rem' }}>Optional</h2>

...
export default function SettingsPage() {
  ...
  const CheckItem = ({ field, label, sublabel }: { field: keyof typeof checks; label: string; sublabel?: string }) => (
    <div onClick={() => toggle(field)}
      style={{ display: 'flex', alignItems: 'flex-start', gap: '0.875rem', padding: '1rem 1.25rem', background: checks[field] ? 'var(--db-bg-card-active)' : 'var(--db-bg-card)', border: `1px solid ${checks[field] ? 'rgba(158,118,59,0.2)' : 'var(--db-border-subtle)'}`, borderRadius: '0.5rem', cursor: alreadySigned ? 'default' : 'pointer', transition: 'all 0.15s', marginBottom: '0.75rem' }}>
      <div style={{ width: '1.25rem', height: '1.25rem', borderRadius: '0.25rem', flexShrink: 0, marginTop: '0.0625rem', border: `1.5px solid ${checks[field] ? '#9e763b' : 'var(--db-text-ghost)'}`, background: checks[field] ? '#9e763b' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.15s' }}>
        {checks[field] && <span style={{ color: '#fff', fontSize: '0.75rem', lineHeight: 1 }}>✓</span>}
      </div>
      <div>
        <p style={{ fontSize: '0.875rem', color: 'var(--db-text)', fontFamily: 'var(--font-sans)', margin: '0 0 0.1875rem', lineHeight: 1.4 }}>{label}</p>
        {sublabel && <p style={{ fontSize: '0.75rem', color: 'var(--db-text-muted)', fontFamily: 'var(--font-sans)', margin: 0, lineHeight: 1.4 }}>{sublabel}</p>}
      </div>
    </div>
  )
  ...
  return (
    <div>
      <div style={{ marginBottom: '2rem' }}>
        ...
        <h1 style={{ fontFamily: 'var(--font-display)', color: 'var(--db-text)', fontSize: 'clamp(1.375rem, 3vw, 1.625rem)', fontWeight: 400, letterSpacing: '0.06em', margin: '0 0 0.375rem' }}>Partner Agreement</h1>
        <p style={{ fontFamily: 'var(--font-serif)', color: 'var(--db-text-muted)', fontSize: '0.875rem', margin: 0 }}>Review and confirm your agreement to list on Palmera.</p>
      </div>
      ...
      <div style={{ background: 'var(--db-bg-card)', border: '1px solid var(--db-border-subtle)', borderRadius: '0.5rem', padding: '1.25rem 1.5rem', marginBottom: '1.75rem' }}>
        <h2 style={{ fontFamily: 'var(--font-serif)', color: 'var(--db-text)', fontSize: '1rem', fontWeight: 400, margin: '0 0 1rem', letterSpacing: '0.04em' }}>What you&apos;re agreeing to</h2>
        {['Palmera handles bookings and customer payments on your behalf.', 'You will confirm bookings within your stated response time.', 'Your Palmera listing price will not be undercut on other booking channels.', 'You agree to Palmera\'s cancellation and refund policies for guests.', 'Palmera will pay out your earnings on your agreed schedule via your chosen method.', 'You may be featured in Palmera curated collections and promotional materials.'].map(term => (
          <div key={term} style={{ display: 'flex', gap: '0.625rem', marginBottom: '0.625rem', alignItems: 'flex-start' }}>
            ...
            <p style={{ fontSize: '0.8125rem', color: 'var(--db-text-muted)', fontFamily: 'var(--font-sans)', margin: 0, lineHeight: 1.5 }}>{term}</p>
          </div>
        ))}
        <div style={{ marginTop: '0.875rem', paddingTop: '0.875rem', borderTop: '1px solid var(--db-border-subtle)' }}>
      ...
      <h2 style={{ fontFamily: 'var(--font-serif)', color: 'var(--db-text)', fontSize: '1.0625rem', fontWeight: 400, margin: '0 0 1rem' }}>Confirm and sign</h2>
      ...
      {!alreadySigned && (
        <button onClick={handleSignoff} disabled={!allChecked || saving}
          style={{ marginTop: '0.5rem', padding: '0.8125rem 2.25rem', background: allChecked ? '#9e763b' : 'var(--db-bg-card)', border: `1px solid ${allChecked ? 'transparent' : 'var(--db-border-subtle)'}`, borderRadius: '0.375rem', color: allChecked ? '#ebe8db' : 'var(--db-text-ghost)', fontSize: '0.875rem', fontFamily: 'var(--font-sans)', letterSpacing: '0.06em', cursor: (!allChecked || saving) ? 'not-allowed' : 'pointer', transition: 'all 0.2s' }}>

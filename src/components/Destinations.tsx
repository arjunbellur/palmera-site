...
const DESTINATIONS = [
  { country: 'Sénégal', city: 'Dakar', image: '/images/ton-toan-dxwt8veyBzQ-unsplash.jpg' },
  { country: 'Maroc', city: 'Marrakesh', image: '/images/paul-macallan-CFKksjYRSQ8-unsplash.jpg' },
  { country: 'Nigeria', city: 'Lagos', image: '/images/gbenga-onalaja-bZC_VAVhoQE-unsplash.jpg' },
]

// Country names in EN
const DESTINATIONS_EN: Record<string, string> = { 'Sénégal': 'Senegal', 'Maroc': 'Morocco', 'Nigeria': 'Nigeria' }
...
export default function Destinations() {
  ...
  const locale = typeof document !== 'undefined' ? (document.cookie.match(/locale=([^;]+)/)?.[1] || 'fr') : 'fr'
    ...
    <section id="makes" style={{ background: 'var(--bg-body)', padding: '5rem 2.5rem 18.75rem' }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
        {DESTINATIONS.map((dest, i) => {
          const countryName = locale === 'fr' ? dest.country : (DESTINATIONS_EN[dest.country] || dest.country)
          return (
            <div key={dest.city} ref={el => { cardRefs.current[i] = el }}
              style={{ position: 'sticky', top: `${3 + i * 0.75}rem`, height: '85vh', minHeight: '33.75rem', borderRadius: '0.75rem', overflow: 'hidden', zIndex: 10 + i, transformOrigin: 'center top', transition: 'transform 0.3s ease, filter 0.3s ease', willChange: 'transform, filter' }}>
              ...
              <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', textAlign: 'center', padding: '5rem 2.5rem 3rem' }}>
                ...
                <h2 style={{ fontFamily: 'var(--font-serif)', fontSize: 'clamp(4.5rem, 12vw, 10rem)', fontWeight: 400, letterSpacing: '-0.1875rem', lineHeight: 0.9, color: '#ebe8db', margin: 0 }}>{dest.city}</h2>
              </div>
            </div>
          )
        })}
